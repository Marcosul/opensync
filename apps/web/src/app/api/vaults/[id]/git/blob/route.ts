...
export type VaultGitBlobResponse = {
  content: string;
  commitHash: string;
};
...
export async function GET(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await backendRequest<VaultGitBlobResponse>(
      `/vaults/${encodeURIComponent(id)}/git/blob${qs}`,
      user,
      { method: "GET" },
    );
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Falha ao ler ficheiro";
